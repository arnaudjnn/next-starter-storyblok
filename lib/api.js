...
export async function getArticle(slug, { locale, preview }) {
  ...
  if(story) {
    ...
    story.content.minReading = Math.ceil(readingTime(render(story.content.content)).minutes)
  }
  return story
}
...
export async function getArticles({ locale, preview, searchTerm=null, perPage=25, page=1, filter={}, byUuids }) {
  ...
  if (story) {
    story.items.map(article => {
      ...
      article.content.minReading = Math.ceil(readingTime(render(article.content.content)).minutes)
    })
  }
  return story
}
